...
public class product_menu {
	...
	void run() {
		...
		System.out.println("Grocery inventory menu:\n" + "1.Insert new product. \n" + "2.Update Product.\n"
				...
				+ "6.Get products in inventory.\n" + "7.Get product details.\n" +"8.Recive products from supplier.\n"
				+"9.Transport quantity of product from storeroom to store\n"+ "10.Category manager.\n"+ "11.Main menu.");
		if(missingList.size()>0)
			System.err.println("There are some product that running out");
		String s = in.nextLine();
		while (!check.check_number_range(s, 1, 11)) {
		...
		case (8): {
			Recive_products_from_supplier();
			;
			break;
		}
		case (9): {
			Transport_quantity_of_product_from_storeroom_to_store();
			;
			break;
		}
		case (10): {
																			...
																			if (faultyProductInStoreroom < 0) {
																				...
																				if (faultyProductInStoreroom > storeroomQuantity) {
																					...
																					if (s.trim().equals("$")) {
																						...
																						double buyPrice = Double.parseDouble(s);
																						if (buyPrice < 0) {
																							System.out.println(
																									"Illegal input, faulty Product In Store must be greater to zero");
																							run();
																							...
																							if (s.trim().equals("$")) {
																								...
																								double sellPrice = Double.parseDouble(s);
																								if (sellPrice < 0) {
																									System.out.println(
																											"Illegal input, faulty Product In Store must be greater to zero");
																									run();
		...
		if (s.trim().equals("$")) {
			...
			if (!bl.isProductExistById(Integer.parseInt(s))) {
				...
				System.out.println("Product update menu:\n" + "1. Update Name.\n" + "2. Update delivery Time.\n"
						...
						+ "12.Update sell Price\n" + "13.Update category ID\n" + "14.Back to Product menu.\n");
				s = in.nextLine();
	...
	public void GetDefectiveProducts() {
		...
		if (defectiveList == null) {
			...
			for (Product p : defectiveList) {
				System.out.println("Product barcode: " + p.getBarcodeProduct() + ", " + "product name: " + p.getProductName()
	...
	public void Recive_products_from_supplier(){
		System.out.println("Insert id product you want to update:\nif it's new product in store insert 'new'\nIf you wish to return to the product menu press $ ");
		String s=in.nextLine();
		while(!(s.equals("$"))&&!(check.checkOnlyNumbers(s)&&!(s.equals("new")))){
			System.out.println("Illegal input, please try again.");
			s = in.nextLine();
		}
		if(s.equals("$"))
			...
			if(s.equals("new"))
				...
				Product toUpdate=bl.getProductById(Integer.parseInt(s));
				if(toUpdate==null){
					System.out.println("The product is not exists");
					Recive_products_from_supplier();
				}
				else{
					System.out.println("Insert amount of product got from supllier");
					s=in.nextLine();
					while(!(check.checkOnlyNumbers(s))){
						System.out.println("Illegal input, please try again.");
						s = in.nextLine();
					}
					toUpdate.setStoreroomQuantity(toUpdate.getStoreroomQuantity()+Integer.parseInt(s));
					bl.updateProductTable(toUpdate);
					System.out.println("The inventory was updated successfully!");
					Recive_products_from_supplier();
				}
			}
		}
	...
	public void Transport_quantity_of_product_from_storeroom_to_store(){
		System.out.println("Insert id product you want to update:\nIf you wish to return to the product menu press $ ");
		String s=in.nextLine();
		while(!(s.equals("$"))&&!(check.checkOnlyNumbers(s))){
			System.out.println("Illegal input, please try again.");
			s = in.nextLine();
		}
		if(s.equals("$"))
				...
				if(toUpdate==null){
					...
					System.out.println("Insert amount of product you want to transport:");
					s=in.nextLine();
					while(!(check.checkOnlyNumbers(s))){
						System.out.println("Illegal input, please try again.");
						s = in.nextLine();
					}
					if(Integer.parseInt(s)>toUpdate.getStoreroomQuantity()){
						System.out.println("Illegal input, you cannot transport more quantity than you have in the storeroom!");
						Transport_quantity_of_product_from_storeroom_to_store();
					}
					else{
						toUpdate.setStoreroomQuantity(toUpdate.getStoreroomQuantity()-Integer.parseInt(s));
					toUpdate.setStoreQuantity(toUpdate.getStoreQuantity()+Integer.parseInt(s));
					bl.updateProductTable(toUpdate);
					System.out.println("The inventory was updated successfully!");
					Transport_quantity_of_product_from_storeroom_to_store();
					}
